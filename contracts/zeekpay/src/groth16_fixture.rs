// @generated by circuits/scripts/convert-to-soroban.mjs. DO NOT EDIT the
// constants below. The prose above the first `pub const` IS preserved across
// re-pins (see the --rs option), so it is the record of which proof this file
// freezes.
//
// Real Groth16/BLS12-381 vk + proof for the 7-public-input Pedersen-shape
// claim circuit (circuits/src/claim.circom), encoded in Soroban bls12_381
// byte layout. Public, throwaway-setup data (safe to commit).
//
// Public-input order, LOCKED, must match `derive_public_inputs` in lib.rs:
//   [root, nullifier, recipientDigest, amount, tokenId,
//    amountCommitmentX, amountCommitmentY]
// PUBS therefore has 7 entries and IC has 8 (IC[0] + IC[1..7]). Contract and
// fixture must move together: `verifier::verify` rejects an IC/pubs length
// mismatch before any pairing math, so a half-done change fails closed rather
// than verifying something it should not.
//
// Re-pin from the repo root, after re-running the prover:
...
// The bare invocation (FORCE_FIXTURE=1, no --rs) writes the same constants and
// additionally rewrites circuits/build/groth16_soroban.json, the file `set_vk`
// is fed from.
//
// PROOF_A/PROOF_B/PROOF_C and PUBS are not reproducible: Groth16 blinds each
// proof, and PUBS moves whenever gen-test-proof.mjs inputs change. Re-pin
// deliberately, not to tidy a diff.
//
// Last re-pinned 2026-09-14, for the 7-input Pedersen claim shape.
